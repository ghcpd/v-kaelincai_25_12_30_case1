...
class ShiftValidator:
    ...
    def validate_new_shift(self, new_shift: Shift) -> bool:
        ...
        """
        # Ensure same-day rule is enforced
        self._validate_same_day_shifts(new_shift)

        # Run other validations and aggregate errors so that when multiple rules
        # would be violated we report all of them (helps tests that match
        # different messages on the same input).
        errors = []
        ...
        if errors:
            # Raise a single ValidationError containing all issues found
